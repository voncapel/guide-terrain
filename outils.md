...
<p class="form-header"><strong>Nom :</strong> {% include form-fill.html ch=46 %} <strong>Date :</strong> {% include form-fill.html ch=20 %}</p>
...
{% include form-lines.html count=2 %}
...
{% include form-lines.html count=1 %}
...
{% include form-lines.html count=3 %}
...
_NB : les éléments issus de cette évaluation conjointe ne peuvent en aucun cas être utilisés dans une évaluation académique, disciplinaire ou administrative de la chercheuse._

<p class="form-header"><span class="form-field-inline"><strong>Chercheuse :</strong> {% include form-fill.html ch=32 %}</span> <span class="form-field-inline"><strong>Encadrante :</strong> {% include form-fill.html ch=32 %}</span> <span class="form-field-inline"><strong>Date :</strong> {% include form-fill.html ch=14 %}</span></p>
...
<p class="form-line">Essentielles : {% include form-fill.html ch=86 class="form-fill--stretch" %}</p>

{% include form-lines.html count=1 %}

<p class="form-line">Secondaires : {% include form-fill.html ch=87 class="form-fill--stretch" %}</p>

{% include form-lines.html count=1 %}

<p class="form-line">Substituables : {% include form-fill.html ch=86 class="form-fill--stretch" %}</p>
...
<section class="tool-2-compact" markdown="1">
...
<p class="form-line">Chercheuse : {% include form-fill.html ch=86 class="form-fill--stretch" %}</p>

{% include form-lines.html count=1 %}

<p class="form-line">Encadrante : {% include form-fill.html ch=86 class="form-fill--stretch" %}</p>
...
<section class="form-lines-below" markdown="1">
...
<p class="form-line">Acceptable : {% include form-fill.html ch=88 class="form-fill--stretch" %}</p>

<p class="form-line">Tolérable : {% include form-fill.html ch=89 class="form-fill--stretch" %}</p>

<p class="form-line">Inacceptable (arrêt) : {% include form-fill.html ch=79 class="form-fill--stretch" %}</p>
...
<p class="form-line">Fréquence convenue : {% include form-fill.html ch=88 class="form-fill--stretch" %}</p>

<p class="form-line">Modalités (présentiel / visio) : {% include form-fill.html ch=79 class="form-fill--stretch" %}</p>

**Assurance et mission**

<p class="form-line">Responsable : {% include form-fill.html ch=88 class="form-fill--stretch" %}</p>

<p class="form-line">Vérifications effectuées : {% include form-fill.html ch=79 class="form-fill--stretch" %}</p>

**Contacts urgents**

<p class="form-line">Liste validée et à jour : {% include form-fill.html ch=81 class="form-fill--stretch" %}</p>

{% include form-lines.html count=1 %}

**Soutien psychologique**

<p class="form-line">Ressources identifiées : {% include form-fill.html ch=88 class="form-fill--stretch" %}</p>

<p class="form-line">Modalités d’accès : {% include form-fill.html ch=79 class="form-fill--stretch" %}</p>
...
<p class="form-line">Nom complet de la chercheuse : {% include form-fill.html ch=69 %}</p>

<p class="form-line">Nationalité(s), documents de voyage utilisés : {% include form-fill.html ch=56 %}</p>

<p class="form-line">Terrain, Pays, Lieu : {% include form-fill.html ch=79 %}</p>

<p class="form-line">Dates du séjour : {% include form-fill.html ch=82 %}</p>

<p class="form-line">Nom et coordonnées de l’encadrante principale : {% include form-fill.html ch=77 %}</p>

<p class="form-line">Nom et coordonnées de la direction du laboratoire : {% include form-fill.html ch=73 %}</p>

<p class="form-line">Référents de sécurité institutionnel : {% include form-fill.html ch=80 %}</p>

<p class="form-line">Contacts d'urgence locaux (police, ambulance, ambassade) :</p>

{% include form-lines.html count=2 %}

<p class="form-line">Personne de confiance (hors institution) : {% include form-fill.html ch=80 %}</p>

#### 2. Typologie des incidents et mesures préventives 

<p class="form-line">Physique, environnemental : {% include form-fill.html ch=51 %}</p>

<p class="form-line">Politique, sécuritaire : {% include form-fill.html ch=51 %}</p>

<p class="form-line">Interactionnel (harcèlement, pressions, intimidations) : {% include form-fill.html ch=66 %}</p>

<p class="form-line">Lieux d'entretien, vulnérabilité spatiale : {% include form-fill.html ch=66 %}</p>

<p class="form-line">Social, culturel, lié à la positionnalité : {% include form-fill.html ch=60 %}</p>

<p class="form-line">Psychique, émotionnel : {% include form-fill.html ch=60 %}</p>

<p class="form-line">Institutionnel : {% include form-fill.html ch=60 %}</p>

<p class="form-line">Numérique, données, technologies : {% include form-fill.html ch=60 %}</p>

<p class="form-line">Vie personnelle et familiale : {% include form-fill.html ch=60 %}</p>


#### 3. Protocole de retrait immédiat

<p class="form-line">Quitter le lieu, se mettre en sécurité (comment ?) : {% include form-fill.html ch=63 %}</p>

<p class="form-line">Lieux sûrs identifiés à proximité : {% include form-fill.html ch=66 %}</p>

<p class="form-line">Personne à contacter immédiatement (local) : {% include form-fill.html ch=77 %}</p>

<p class="form-line">Suspension immédiate de la collecte de données : {% include form-fill.html ch=77 %}</p>

<p class="form-line">Procédure de sauvegarde, isolement, destruction temporaire des données sensibles : {% include form-fill.html ch=77 %}</p>

<p class="form-line">Moyen de transport de repli identifié : {% include form-fill.html ch=77 %}</p>

#### 4. Gestion des données sensibles

<p class="form-line">Sauvegarde prévue (où, comment?) : {% include form-fill.html ch=67 %}</p>

<p class="form-line">Chiffrement prévu : {% include form-fill.html ch=67 %}</p>

<p class="form-line">Supports numériques utilisés (téléphone, dictaphone, ordinateur) : {% include form-fill.html ch=67 %}</p>

<p class="form-line">Plan en cas de saisie, perte, compromission : {% include form-fill.html ch=67 %}</p>

<p class="form-line">Rapport interne de signalement prévu ? (oui/non) : {% include form-fill.html ch=67 %}</p>
...
- [ ] Enregistrement sur le [Fil d'Ariane](https://fildariane.diplomatie.gouv.fr/fildariane-internet/accueil){: .tool-link-external }
...
<p class="form-header"><strong>Date :</strong> {% include form-fill.html ch=16 %} <strong>Jour de terrain n° :</strong> {% include form-fill.html ch=8 %}</p>

<p class="signals-disclaimer"><strong>NB :</strong> cette checklist ne remplace pas une évaluation clinique. Elle est conçue comme outil d’auto-surveillance. Les signaux décrits peuvent varier en intensité et apparaître à différents moments : parfois peu perceptibles sur le moment, notamment en situation de stress intense, ils peuvent se manifester pendant le terrain, dans les semaines qui suivent ou plusieurs mois après le retour. En cas de doute ou de persistance des symptômes, contactez une professionnelle de santé.</p>

<div class="signals-tool">
  <section class="signals-stage signals-stage--1">
    <h3 class="signals-stage-title">Auto-évaluation rapide <span>(à faire chaque soir)</span></h3>
    <ul class="signals-list">
      <li>Mon niveau d’énergie est-il insuffisant pour continuer demain ?</li>
      <li>Ai-je réussi à identifier trois moments neutres ou positifs aujourd’hui ?</li>
      <li>Est-ce que je me suis sentie en sécurité à un moment donné aujourd’hui ?</li>
      <li>Ai-je évité une situation que j’aurais dû arrêter plus tôt ?</li>
    </ul>
  </section>

  <section class="signals-stage signals-stage--2">
    <h3 class="signals-stage-title">Signaux à surveiller</h3>
    <div class="signals-grid">
      <article class="signals-card">
        <h4 class="signals-card-title">Signaux corporels</h4>
        <ul class="signals-list">
          <li>Fatigue extrême persistante malgré le repos, troubles du sommeil</li>
          <li>Tensions musculaires, tremblements</li>
          <li>Maux de tête récurrents</li>
          <li>Perte d’appétit, nausées, troubles digestifs</li>
          <li>Hypersensibilité aux éléments extérieurs (bruits, lumière, etc.)</li>
        </ul>
      </article>

      <article class="signals-card">
        <h4 class="signals-card-title">Signaux cognitifs</h4>
        <ul class="signals-list">
          <li>Difficulté de concentration</li>
          <li>Confusion ou oublis inhabituels</li>
          <li>Pensées envahissantes liées à des scènes ou récits du terrain</li>
          <li>Difficultés à prendre des décisions simples</li>
        </ul>
      </article>

      <article class="signals-card">
        <h4 class="signals-card-title">Signaux émotionnels et relationnels</h4>
        <ul class="signals-list">
          <li>Irritabilité inhabituelle, sautes d’humeur importantes</li>
          <li>Anxiété persistante, crises de panique</li>
          <li>Sentiment paranoïaque</li>
          <li>Difficulté à éprouver des émotions positives</li>
        </ul>
      </article>

      <article class="signals-card">
        <h4 class="signals-card-title">Signaux comportementaux</h4>
        <ul class="signals-list">
          <li>Isolement croissant, évitement des interactions</li>
          <li>Hypervigilance (surveillance excessive de l’environnement)</li>
          <li>Consommation accrue d’alcool ou d’anxiolytiques</li>
          <li>Impulsivité ou comportements à risque</li>
        </ul>
      </article>
    </div>
  </section>

  <section class="signals-stage signals-stage--3">
    <h3 class="signals-stage-title">Signaux rouges : faire une pause et contacter sa personne référente</h3>
    <ul class="signals-list">
      <li>Incapacité à poursuivre la journée</li>
      <li>Impression d’être en danger même dans un lieu objectivement sécurisé</li>
      <li>Épisode de dissociation (impression d’irréalité)</li>
      <li>Pensées intrusives persistantes</li>
      <li>Hypervigilance empêchant le sommeil</li>
      <li>Crainte d’un effondrement émotionnel</li>
    </ul>
  </section>
</div>
